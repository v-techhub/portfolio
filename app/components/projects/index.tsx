  ...
  return (
    <SheetContent className="overflow-y-scroll w-[90dvw] md:w-auto">
      <SheetHeader className="pt-10">
        ...
        <SheetDescription>{projectOverview}</SheetDescription>
      </SheetHeader>
      <section className="flex flex-col gap-10">
        ...
        <div>
          ...
          <div className="flex gap-3 items-center mt-2 flex-wrap">
            {techStack.map((tech) => (
              <legend
                className="bg-gray-300 py-1 text-xs rounded-xl px-3"
                key={tech}
              >
                {tech}
              </legend>
      ...
      <Sheet>
        {listOfProjects.map((project, idx) => (
          <SheetTrigger asChild key={idx}>
            <fieldset
              className="mx-auto w-[90dvw] md:w-[500px] h-[400px] md:h-[285px] rounded-2xl shadow-2xl cursor-pointer"
              onClick={() => handleProjectSelection(idx)}
            >
              <video
                width={100}
                height={300}
